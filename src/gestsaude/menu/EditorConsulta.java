...
public class EditorConsulta extends JDialog {
	...
	/**
	 * Construtor que coloca o titulo na jela sendo chamado pelos outros
	 * construtores
	 * 
	 * @param posicao  onde colocar a janela
	 * @param g        o sistema
	...
	/**
	 * método chamado pela interface para saber se o sns utente é válido. Deve dar
	 * indicações de erro.
	 */
	...
	/**
	 * método chamado pela interface para saber se o id do serviço é válido. Deve
	 * dar indicações de erro.
	 */
	...
	/**
	 * método chamado pela interface para saber se todos os parametros estão
	 * corretos e se a consulta é aceitável. Deve dar indicações de erro.
	 */
	private void testaTudoOk() {
		...
		if (utente == null)
			...
			Consulta c = new Consulta(data, LocalTime.of(h, m), (String) idServico.getSelectedItem(),
					snsUtente.getText());
			int res = 0;
			...
			case GEstSaude.ALTERACAO_INVALIDA:
				...
				break;
			case GEstSaude.CONSULTA_INVALIDA:
				apresentarMensagem("Consulta Invalida!", false);
				break;
			}
		}
	}
	...
	/**
	 * método que vai colocar os identificadores dos serviços, que aceitam
	 * consultas, numa combo box para apresentar ao utilizador
	 * 
	...
	/**
	 * método chamado quando o botão ok é premido fazer aqui as considerações
	 * finais, se necessário
	 */
	protected void okPremido() {
		...
		consultaRes = new Consulta(quando.toLocalDate(), quando.toLocalTime(), (String) idServico.getSelectedItem(),
				snsUtente.getText());
		setVisible(false);
	}

	/**
	 * método chamado quando o botão cancel é premido fazer aqui as considerações
	 * finais, se necessário
	 */
	protected void cancelPremido() {
		if (estaAlterarConsulta && (!gest.getConsultas().contains(consulta)))
	...
	/**
	 * método auxiliar que transforma uma LocalDate numa string com a formatação
	 * portuguesa
	 */
