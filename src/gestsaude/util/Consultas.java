...
public class Consultas {
	...
	public static void addConsultaOrdemData(List<Consulta> cs, Consulta c) {
		...
		if (cs.isEmpty())
			...
			for (Consulta consulta : cs) {
				if (c.getDataConsulta().atTime(c.getHoraConsulta())
						.isAfter(consulta.getDataConsulta().atTime(consulta.getHoraConsulta())))
					idx++;
			}
			cs.add(idx, c);
		}
	...
	public static List<Consulta> getConsultasApos(List<Consulta> cs, LocalDateTime t) {
		...
		return Collections.unmodifiableList(listaConsultasApos);
	}
